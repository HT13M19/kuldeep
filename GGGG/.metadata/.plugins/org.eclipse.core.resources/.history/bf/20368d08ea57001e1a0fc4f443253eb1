...
{	Connection con;
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) 
	{
		try 
		{
			...
			String s8=request.getParameter("incomes");
			String s9=request.getParameter("INCOME");
			String s10=request.getParameter("AGE");
			String s11=request.getParameter("ADDRESS");
			PreparedStatement ps=con.prepareStatement("INSERT INTO user1 (FIRSTNAME, LASTNAME, phone, email, password, gender, account_type,incomes, income, age, address,Bal) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?,'0')");
					ps.setString(1, s1);
					ps.setString(2, s2);
					ps.setString(3, s3);
					ps.setString(4, s4);
